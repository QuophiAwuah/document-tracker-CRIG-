...
<?php
  if(!isset($_SESSION['vam_username']) || !isset($_SESSION['vam_password'])){
     header('Location: login.php');
  }
 ?>

<?php include '../div_query/vam_query.php' ?>
...
<a class="navbar-brand  brand" href="index.php" title="ProBootstrap:Enlight">VA MARTISON CLINIC</a>
</div>
<?php $in = mysqli_num_rows($vam_in)?>
<div id="navbar-collapse" class="navbar-collapse collapse">
          <ul class="nav navbar-nav navbar-right">
          <li><a href="#">Documents~<span class="badge badge-success"><?=$total_vam ?></span></a></li>
          <li><a href="#">Inbox~<span class="badge badge-success"><?=$count_vam2 ?></span></a></li>
          <li><a href="#">Outbox~<span class="badge badge-success"><?=$count_vam1; ?></span></a></li>
          </ul>
</div>
</div>
</nav>
      ...
      <div class="col-md-3">
        <div class="panel">
          <div class="panel-heading">
              All Documents<span class="badge badge-warning"><?=$total_vam ?></span>
            </div>
    ...
    <div id="collapse2" class="panel-collapse collapse">
      <div class="panel-body">
        ...
        <table class="table table-striped table-condensed">
          ...
          <tbody>
            <?php while($vam = mysqli_fetch_assoc($vam_out)): ?>
            <tr class="bg-info">
              <td><?=$vam['doc_id']?></td>
              <td><?=$vam['sender_name']?></td>
              <td><?=$vam['sender_division']?></td>
              <td><?=$vam['receipient_name']?></td>
              <td><?=$vam['reciepient_division']?></td>
              <td><?=$vam['cur_time']?></td>
              <td><?=$vam['cur_date']?></td>
              <td><?=$vam['doc_intro']?></td>
              <td><?=$vam['doc_format']?></td>
            <?php endwhile ?>
          </tr>
          </tbody>
        </table>
    ...
    <div id="collapse1" class="panel-collapse collapse in">
      <div class="panel-body">
        ...
        <table class="table table-striped table-condensed">
          ...
          <tbody>
            <?php while($vam = mysqli_fetch_assoc($vam_in)): ?>
            <tr class="bg-info">
              <td><?=$vam['doc_id']?></td>
              <td><?=$vam['sender_name']?></td>
              <td><?=$vam['sender_division']?></td>
